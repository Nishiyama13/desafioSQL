...
/*D1.Busque todos os produtos com Televisão no nome. Para fazer isso, procure por sql like operator;*/
select * from produtos where nome like '%Televisão%';

/*D2.Busque todos os produtos com preço menor que R$1000,00. Para fazer isso, procure por sql comparison operators;*/
select * from produtos where preco < 100000;

/*D3.Busque todos os produtos com Celular no nome e preço menor que R$2000,00. Para isso, utilize o operador like do item anterior e procure por sql logical operators;*/
select * from produtos where nome like '%Celular%' and preco < 100000;

/*D4.(Desafio²) Busque todos os clientes que não fizeram uma compra. Para isso, procure por sql not in e sql subquery.*/
select * from clientes where id not in (select id_cliente from compras);

/*D5.(Desafio²) Busque todos os produtos comprados por Benício Freire Sampaio. Para isso, procure por sql in e sql subquery;*/
select produtos.* from produtos where id in (select id_produto from compras where id_cliente = (select id from clientes where nome = 'Benício Freire Sampaio'));